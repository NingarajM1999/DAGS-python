...
class MinuteSchedule:
    ...
    def create_dag(self, dag_id, description, start_date, schedule_interval):
        ...
        def second_function_execute(**kwargs):
            ...
            try:
                ...
                response = session.post("http://30.30.30.118:8216/optimize/v1/scheduler/executionDashboardTask", headers=headers)
        ...
        def third_function_execute(**kwargs):
            ...
            try:
                print("Before API call")
                response = session.post("http://30.30.30.118:8216/optimize/v1/scheduler/scheduledExecutionDashboardTask", headers=headers)
                response.raise_for_status()
                data = response.json()
                print("After API call. Response:", data)
            except requests.exceptions.RequestException as e:
        ...
        with dag:
            ...
            second_function_execute_task = PythonOperator(
                task_id="executionDashboardTask",
                python_callable=second_function_execute,
                provide_context=True
            )
            third_function_execute_task = PythonOperator(
                task_id="scheduledExecutionDashboardTask",
    ...
    def create_dags_with_tasks(self):
        ...
        # Create DAG 1 - Schedule every 1 minute
        dashBoard = self.create_dag(
            dag_id='Schedule_minute1',
            description='This is DAG running for one minute',
...
dags_with_tasks=minuteSchedule.create_dags_with_tasks()
dag = dags_with_tasks[0]
task = dag.get_task("sign_in_task")
task1 = dag.get_task("executionDashboardTask")
task2=dag.get_task("scheduledExecutionDashboardTask")
